...
type GameFilter = "ALL" | "DICE" | "COIN_TOSS";
...
function isWon(game: Game) {
  return Number(game.payoutPaise) > 0;
}
...
export default function GameHistoryPage() {
  ...
  const stats = useMemo(() => {
    const wins = games.filter(isWon).length;
  ...
  const filteredGames = useMemo(() => {
    return games.filter((game) => {
      const matchesGame = gameFilter === "ALL" || game.type === gameFilter;
      const matchesResult = resultFilter === "ALL" || (resultFilter === "WON" ? isWon(game) : !isWon(game));
      return matchesGame && matchesResult;
    });
  }, [games, gameFilter, resultFilter]);
  ...
  function changeGameFilter(value: GameFilter) {
    setGameFilter(value);
    setPage(1);
  }
    ...
    <main className="min-h-screen bg-[#07080b] text-white">
      <div className="mx-auto max-w-6xl px-5 pb-12 sm:px-8">
        ...
        <section className="mt-5 rounded-3xl border border-white/10 bg-white/[0.035] shadow-2xl">
          <div className="flex flex-col gap-5 border-b border-white/10 px-5 py-5 sm:px-7">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap gap-2">
                {(["ALL", "DICE", "COIN_TOSS"] as const).map((filter) => (
                  <button key={filter} type="button" onClick={() => changeGameFilter(filter)} className={`rounded-xl border px-3.5 py-2 text-xs font-bold transition ${gameFilter === filter ? "border-white bg-white text-black" : "border-white/10 bg-black/15 text-white/55 hover:border-white/25 hover:text-white"}`}>
                    {filter === "ALL" ? "All games" : filter === "DICE" ? "Dice Roll" : "Coin Toss"}
                  </button>
                ))}
              </div>
              <div className="flex flex-wrap gap-2">
                {(["ALL", "WON", "LOST"] as const).map((filter) => (
                  <button key={filter} type="button" onClick={() => changeResultFilter(filter)} className={`rounded-xl border px-3.5 py-2 text-xs font-bold transition ${resultFilter === filter ? "border-white bg-white text-black" : "border-white/10 bg-black/15 text-white/55 hover:border-white/25 hover:text-white"}`}>
                    {filter === "ALL" ? "All results" : filter === "WON" ? "Won" : "Lost"}
                  </button>
                ))}
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="px-5 py-16 text-center"><p className="text-lg font-bold">No matching rounds</p><p className="mt-2 text-sm text-white/40">Try a different game or result filter.</p><button type="button" onClick={() => { changeGameFilter("ALL"); changeResultFilter("ALL"); }} className="mt-5 rounded-xl border border-white/15 px-4 py-3 text-sm font-bold hover:border-white/30">Clear filters</button></div>
          ) : (
            <>
              ...
              <div className="flex flex-col gap-3 border-t border-white/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
                <p className="text-xs text-white/35">Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filteredGames.length)} of {filteredGames.length} matching rounds</p>
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1} className="rounded-xl border border-white/10 px-3 py-2 text-xs font-bold hover:border-white/25 disabled:cursor-not-allowed disabled:opacity-30">← Previous</button>
                  <span className="min-w-20 text-center text-xs font-bold text-white/45">Page {page} of {pageCount}</span>
                  <button type="button" onClick={() => setPage((current) => Math.min(pageCount, current + 1))} disabled={page === pageCount} className="rounded-xl border border-white/10 px-3 py-2 text-xs font-bold hover:border-white/25 disabled:cursor-not-allowed disabled:opacity-30">Next →</button>
                </div>
              </div>
            </>
          )}
