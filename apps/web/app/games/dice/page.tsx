...
function DieFace({ value }: { value: number }) {
  ...
  return <div className="relative flex h-32 w-32 rotate-[-8deg] items-center justify-center rounded-[1.75rem] border border-white/30 bg-white text-7xl font-black text-black shadow-[0_25px_60px_rgba(0,0,0,0.45)] sm:h-40 sm:w-40 sm:text-8xl"><span className="tracking-[0.05em]">{pips[value]}</span></div>;
}
...
export default function DicePage() {
  ...
  function resetRound() { setResult(null); setError(""); setStake(""); setRollingFace(1); }
    ...
    <main className="min-h-screen bg-[#07080b] text-white">
      <div className="mx-auto max-w-4xl px-5 pb-12 sm:px-8">
        ...
        <form onSubmit={playDice} className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:p-8">
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-6">{faces.map((face) => <button key={face} type="button" onClick={() => { setSelected(face); setResult(null); setError(""); }} disabled={loading} className={`aspect-square rounded-2xl border text-2xl font-black transition sm:text-3xl ${selected === face ? "border-white bg-white text-black" : "border-white/10 bg-white/[0.04] hover:border-white/25"} disabled:cursor-not-allowed disabled:opacity-60`} aria-pressed={selected === face}>{face}</button>)}</div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-black/20 p-4"><p className="text-xs text-white/35">Your prediction</p><p className="mt-1 text-lg font-bold">{selected ? `Number ${selected}` : "Not selected"}</p></div><div className="rounded-2xl bg-black/20 p-4"><p className="text-xs text-white/35">Win payout</p><p className="mt-1 text-lg font-bold">2.5× stake</p></div></div>
          ...
          {result && <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-white/40">Round result</p><p className="mt-2 text-3xl font-black">{result.won ? "You won!" : "Better luck next time"}</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><div><p className="text-white/35">Prediction</p><p className="mt-1 font-bold">{result.prediction}</p></div><div><p className="text-white/35">Result</p><p className="mt-1 font-bold">{result.result}</p></div><div><p className="text-white/35">Payout</p><p className="mt-1 font-bold">₹{(Number(result.payoutPaise) / 100).toFixed(2)}</p></div><div><p className="text-white/35">Balance</p><p className="mt-1 font-bold">₹{(Number(result.balancePaise) / 100).toFixed(2)}</p></div></div><button type="button" onClick={resetRound} className="mt-5 w-full rounded-2xl border border-white/15 px-5 py-3 text-sm font-bold hover:border-white/30">Play another round</button></div>}
          {!result && <button type="submit" disabled={!selected || !stake || loading} className="mt-5 w-full rounded-2xl bg-white px-5 py-4 text-sm font-black text-black transition hover:bg-white/90 disabled:cursor-not-allowed disabled:opacity-30">{loading ? "Rolling..." : selected ? "Roll Dice" : "Select a number"}</button>}
          <p className="mt-4 text-center text-xs leading-5 text-white/30">The outcome is generated on the server using a secure random number generator. The animation only reveals the server result; it does not control it.</p>
        </form>
      </div>
