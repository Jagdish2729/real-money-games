  ...
  return (
    <main className="min-h-screen bg-[#07080b] text-white">
      <div className="mx-auto max-w-3xl px-5 pb-12 sm:px-8">
        ...
        {submitted ? (
          <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-6 sm:p-8"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-xl font-black text-black">✓</div><h2 className="mt-6 text-2xl font-bold">Deposit request submitted</h2><p className="mt-3 text-sm leading-6 text-white/45">Your request is pending manual verification. The wallet balance will be updated only after the payment is approved.</p><div className="mt-6 flex gap-3"><a href="/wallet" className="rounded-xl bg-white px-4 py-3 text-sm font-bold text-black">View wallet</a><button onClick={() => setSubmitted(false)} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold hover:border-white/30">Submit another request</button></div></section>
        ) : (
          <div className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]">
            ...
            <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.16em] text-white/35">Step 2</p><h2 className="mt-2 text-xl font-bold">Submit payment details</h2><form onSubmit={handleSubmit} className="mt-6 space-y-5">
              <label className="block"><span className="text-sm font-semibold">Amount</span><div className="mt-2 flex items-center rounded-xl border border-white/10 bg-black/20 px-4 focus-within:border-white/30"><span className="text-white/45">₹</span><input type="number" min="1" step="1" value={amount} onChange={event => setAmount(event.target.value)} placeholder="Enter amount" className="w-full bg-transparent px-3 py-3 text-sm outline-none placeholder:text-white/25" required /></div></label>
              <label className="block"><span className="text-sm font-semibold">UTR / transaction reference</span><input value={utr} onChange={event => setUtr(event.target.value)} placeholder="Enter payment reference" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm outline-none placeholder:text-white/25 focus:border-white/30" required /></label>
              <label className="block"><span className="text-sm font-semibold">Payment proof</span><input type="file" accept="image/*" onChange={event => setProofFile(event.target.files?.[0] ?? null)} className="mt-2 block w-full rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white/55 file:mr-3 file:rounded-lg file:border-0 file:bg-white file:px-3 file:py-2 file:text-xs file:font-bold file:text-black" /><span className="mt-2 block text-xs text-white/30">{proofFile ? `${proofFile.name} selected` : "Proof upload will be connected to secure storage next."}</span></label>
              {error && <div className="rounded-2xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200">{error}</div>}
              <div className="rounded-2xl border border-white/10 bg-black/15 p-4 text-xs leading-5 text-white/40">Deposits are credited only after admin verification. This request does not change your wallet balance directly.</div>
              <button disabled={loading} type="submit" className="w-full rounded-xl bg-white px-4 py-3.5 text-sm font-bold text-black hover:bg-white/90 disabled:cursor-not-allowed disabled:opacity-50">{loading ? "Submitting..." : "Submit deposit request"}</button>
            </form></section>
          </div>
        )}
      </div>
